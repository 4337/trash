		...
		export {
			...
			void raw_release(T* ptr) noexcept {
				...
				for (size_t i = 0; i < sz; i++) {
					...
					if (ptr->policies[i].raw != nullptr) {

						ptr->policies[i].raw->Release();
						ptr->policies[i].raw = nullptr;
			...
			class AuthorizedApplications {  
				  ...
				  /// <summary>
				  /// COM - prawdopodobnie powinien byæ shared ze zliczaniem referencji i custom deleterem, ale
				  /// poniewa¿ zablokowaliœmy mo¿liwoœæ kopiowania to zostawiamy jako "raw" pointer.
				  /// </summary>
				  INetFwAuthorizedApplications* aa;
